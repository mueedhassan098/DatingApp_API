...
namespace APIGateWay.Helpers
{
    public class AutoMapperProfiles : Profile
    {
        public AutoMapperProfiles()
        {
            ...
            //CreateMap<Message, MessageDto>()
    .ForMember(dest => dest.MessageSent, opt => opt.MapFrom(src =>
        DateTime.SpecifyKind(src.MessageSent, DateTimeKind.Utc)))
    .ForMember(dest => dest.DateRead, opt => opt.MapFrom(src =>
        src.DateRead.HasValue
            ? DateTime.SpecifyKind(src.DateRead.Value, DateTimeKind.Utc)
            : (DateTime?)null
    ));
